...
{
    twi_init();
    twi_attachSlaveTxEvent(nop1);
    twi_attachSlaveRxEvent(nop2);
    return ATCA_SUCCESS;
}
...
#define IDLE_ADDR  0x02
#define SLEEP_ADDR 0x01

static ATCA_STATUS sleep_or_idle(ATCAIface iface, uint8_t cmd_byte)
{
    const ATCAIfaceCfg *cfg = atgetifacecfg(iface);
    ...
    return ret == 0 ? ATCA_SUCCESS : ATCA_COMM_FAIL;
}

ATCA_STATUS hal_i2c_idle(ATCAIface iface)
{
    const ATCA_STATUS status = sleep_or_idle(iface, IDLE_ADDR);
...
{
    return sleep_or_idle(iface, SLEEP_ADDR);
}
